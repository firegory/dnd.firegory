...
export function FlatDetail({ entry }: Readonly<{ entry: FlatDetailValue }>) {
  ...
  return <article className="flat-detail">
    ...
    {entry.relations.length ? <section><h2>{language === "ru" ? "Связанные записи" : "Related entries"}</h2><ul>{entry.relations.map((relation) => <li key={`${relation.direction}-${relation.type}-${relation.entryId}`}><Link href={compendiumEntryRoute(relation.entryType, relation.entryId, language)}>{relation.title}</Link> · {relation.type}</li>)}</ul></section> : null}
