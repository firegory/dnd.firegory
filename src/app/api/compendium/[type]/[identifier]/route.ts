...
import { parseFlatType } from "../../../../../server/compendium/flat-http";
import { FlatNotFoundError, FlatReadInputError, FlatReadService } from "../../../../../server/compendium/flat-read-service";

export async function GET(_request: Request, context: { params: Promise<{ type: string; identifier: string }> }): Promise<NextResponse> {
  const user = await getCurrentUser(); if (!user) return authenticationRequired();
  try { const params = await context.params; return NextResponse.json(await new FlatReadService().get(retrievalUser(user), parseFlatType(params.type), decodeURIComponent(params.identifier))); }
