...
export function Toggle({ label, value, options, onChange, className = "", disabled = false }: { label: string; value: string; options: readonly [ToggleOption, ToggleOption]; onChange: (value: string) => void; className?: string; disabled?: boolean }) {
  return (
    <div className={`flex flex-col gap-1.5 ${className}`}>
      ...
      <div role="radiogroup" aria-label={label} aria-disabled={disabled} className={`relative grid min-w-32 grid-cols-2 rounded-full border border-border bg-primary/60 p-1 ${disabled ? "opacity-60" : ""}`}>
        ...
        {options.map((option) => {
          ...
          return <button key={option.value} type="button" role="radio" aria-checked={active} disabled={disabled} onClick={() => onChange(option.value)} className={`relative z-10 rounded-full px-2 py-1.5 text-sm font-semibold transition-colors disabled:cursor-wait ${active ? "text-primary" : "text-text-muted hover:text-text-primary"}`}>{option.label}</button>;
        })}
      </div>
    </div>
  );
