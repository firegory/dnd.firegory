      ...
      SELECT flat.*, coalesce((
        ...
        FROM (
          ...
          WHERE owner.entry_type = flat.entry_type::compendium_entry_type
            ...
            AND EXISTS (SELECT 1 FROM compendium_import_links evidence JOIN accessible_relation_evidence authorized ON authorized.version_id = evidence.evidence_version_id WHERE evidence.relation_id = rel.id)
          UNION ALL
        ...
        JOIN accessible_relation_versions target ON target.entry_id = target_entry.entry_type::text || '-' || target_entry.canonical_key
          AND target.edition = flat.edition AND target.language = flat.language AND target.source_rank = 1
      ), '[]'::jsonb) AS relations
...
function boundarySql(user: RetrievalUser, type: FlatEntryType, selection: RetrievalSelection | FlatListOptions): { sql: string; params: unknown[]; typeParam: number } {
  ...
  return { params, typeParam: params.length, sql: `WITH accessible_flat_versions AS MATERIALIZED (
    ...
    SELECT version.id AS version_id FROM compendium_versions version
