...
export class FlatReadService {
  ...
  async list(user: RetrievalUser, type: FlatEntryType, options: FlatListOptions = {}): Promise<Readonly<{ entries: readonly FlatListEntry[]; count: number; nextCursor: string | null }>> {
    ...
    const result = await this.db.query<FlatRow>(`${boundary.sql} SELECT flat.* FROM accessible_flat flat JOIN nfs_index_entries indexed ON indexed.id = flat.index_id AND indexed.lifecycle = 'active' AND indexed.entry_type = $${boundary.typeParam} WHERE ${filters} ORDER BY indexed.edition, indexed.language, lower(indexed.name) COLLATE "C", indexed.entry_id LIMIT $${boundary.params.length}`, boundary.params);
    ...
    return { entries: visible.map(mapListEntry), count, nextCursor: result.rows.length > limit && last ? encodeCursor(last.edition, last.language, last.sort_title, last.entry_id) : null };
  }

  async count(user: RetrievalUser, type: FlatEntryType, options: Omit<FlatListOptions, "cursor" | "limit"> = {}): Promise<number> {
    validateOptions(options);
    const boundary = boundarySql(user, type, options);
    const result = await this.db.query<{ count: string } & QueryResultRow>(`${boundary.sql} SELECT count(*)::text AS count FROM accessible_flat flat JOIN nfs_index_entries indexed ON indexed.id = flat.index_id AND indexed.lifecycle = 'active' AND indexed.entry_type = $${boundary.typeParam} WHERE ${flatFilters(boundary.params, options)}`, boundary.params);
  ...
  async get(user: RetrievalUser, type: FlatEntryType, identifier: string, selection: RetrievalSelection = {}): Promise<FlatDetail> {
    ...
    const result = await this.db.query<FlatRow>(`${boundary.sql}, matched_flat AS MATERIALIZED (
      SELECT flat.*, CASE WHEN flat.entry_id = $${boundary.params.length} THEN 0 ELSE 1 END AS identifier_rank
    ...
    ), preferred_flat AS MATERIALIZED (
      SELECT matched.*, min(identifier_rank) OVER () AS best_rank FROM matched_flat matched
    ), selected_flat AS MATERIALIZED (
      SELECT preferred.*, count(*) OVER () AS match_count FROM preferred_flat preferred WHERE identifier_rank = best_rank
    )
      SELECT flat.*, coalesce((
        SELECT jsonb_agg(jsonb_build_object('type', relation.relation_type, 'direction', relation.direction,
          'entryId', CASE WHEN relational_target.entry_type::text IN ('spell', 'feat', 'background', 'item', 'equipment', 'glossary') THEN nfs_target.entry_id ELSE relational_target.entry_id::text END,
          'entryType', CASE WHEN relational_target.entry_type::text IN ('spell', 'feat', 'background', 'item', 'equipment', 'glossary') THEN nfs_target.entry_type ELSE relational_target.entry_type::text END,
          'title', coalesce(nfs_target.name, relational_target.title))
          ORDER BY coalesce(nfs_target.name, relational_target.title), relational_target.entry_id)
        FROM (
      ...
      FROM selected_flat flat WHERE flat.match_count = 1 LIMIT 1`, boundary.params);
    if (!result.rows[0]) throw new FlatNotFoundError();
    return mapDetail(result.rows[0]);
  }
}

function boundarySql(user: RetrievalUser, type: FlatEntryType, selection: RetrievalSelection | FlatListOptions): { sql: string; params: unknown[]; typeParam: number } {
  ...
  return { params, typeParam: params.length, sql: `WITH accessible_flat_versions AS MATERIALIZED (
    ...
    JOIN compendium_names slug ON slug.version_id = version.id AND slug.kind = 'slug'
    JOIN sources s ON s.id = version.source_id
    JOIN files f ON f.id = version.file_id AND f.source_id = s.id
    WHERE ${access.sql} AND version.lifecycle = 'published' AND revision.lifecycle = 'published'
    ...
    SELECT flat_version.*, (SELECT jsonb_agg(jsonb_build_object('sourceId', version.source_id, 'title', version.source_title,
      ...
      WHERE version.entry_id = flat_version.entry_id AND version.edition = flat_version.edition AND version.language = flat_version.language) AS source_versions
...
function flatFilters(params: unknown[], options: FlatListOptions | Omit<FlatListOptions, "cursor" | "limit">): string {
  ...
  for (const [key, value, operator] of [["prerequisite-level", options.minLevel, ">="], ["prerequisite-level", options.maxLevel, "<="], ["cost-cp", options.minCost, ">="], ["cost-cp", options.maxCost, "<="], ["weight-lb", options.minWeight, ">="], ["weight-lb", options.maxWeight, "<="]] as const) {
    if (value !== undefined) filters.push(`nfs_index_typed_number(indexed.typed_fields, '${key}') ${operator} $${params.push(value)}`);
  }
  if (options.related) contains("related-terms", [options.related]);
  if (options.query) filters.push(`(indexed.name ILIKE $${params.push(`%${escapeLike(options.query)}%`)} ESCAPE '\\' OR EXISTS (SELECT 1 FROM jsonb_array_elements_text(indexed.aliases) alias WHERE alias ILIKE $${params.length} ESCAPE '\\'))`);
  if ("cursor" in options && options.cursor) { const cursor = decodeCursor(options.cursor); params.push(cursor.edition, cursor.language, cursor.title, cursor.id); filters.push(`(indexed.edition, indexed.language, lower(indexed.name) COLLATE "C", indexed.entry_id) > ($${params.length - 3}, $${params.length - 2}, $${params.length - 1} COLLATE "C", $${params.length})`); }
...
function encodeCursor(edition: string, language: string, title: string, id: string): string { return Buffer.from(JSON.stringify({ v: 1, edition, language, title, id }), "utf8").toString("base64url"); }
function decodeCursor(value: string): { edition: "5e" | "5.5e"; language: "en" | "ru"; title: string; id: string } { try { if (value.length > 512) throw new Error(); const decoded = JSON.parse(Buffer.from(value, "base64url").toString("utf8")); if (!isRecord(decoded) || decoded.v !== 1 || !["5e", "5.5e"].includes(String(decoded.edition)) || !["en", "ru"].includes(String(decoded.language)) || typeof decoded.title !== "string" || typeof decoded.id !== "string" || !STABLE_ID.test(decoded.id)) throw new Error(); return { edition: decoded.edition as "5e" | "5.5e", language: decoded.language as "en" | "ru", title: decoded.title, id: decoded.id }; } catch { throw new FlatReadInputError("Invalid flat entry cursor."); } }
