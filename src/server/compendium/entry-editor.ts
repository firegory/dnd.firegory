...
function camelProjection(value: Record<string,unknown>): Record<string,unknown> { const aliases:Record<string,string>={casting_time:"castingTime",range_text:"range",creature_type:"creatureType",armor_classes:"armorClass",hit_points_detail:"hitPoints",challenge_rating:"challengeRating",speeds:"speeds",damage_resistances:"damageResistances",damage_immunities:"damageImmunities",condition_immunities:"conditionImmunities",passive_perception:"passivePerception",bonus_actions:"bonusActions",legendary_actions:"legendaryActions",requires_attunement:"requiresAttunement",hit_die:"hitDie",primary_ability:"primaryAbility",spellcasting_ability:"spellcastingAbility",feature_kind:"featureKind",ability_scores:"abilityScores",skill_proficiencies:"skillProficiencies",prerequisite_level:"prerequisiteLevel",prerequisite_text:"prerequisiteText",cost_cp:"costCp",weight_lb:"weightLb",extension_data:"extensionData"}; const creature = value.challenge_rating_numerator != null && value.challenge_rating_denominator != null; return Object.fromEntries(Object.entries(value).filter(([key])=>!creature||!["armor_class","hit_points","speed","challenge_rating","challenge_rating_numerator","challenge_rating_denominator"].includes(key)).map(([key,item])=>[aliases[key]??key,item]).concat(creature ? [["challengeRating",{numerator:Number(value.challenge_rating_numerator),denominator:Number(value.challenge_rating_denominator)}]] : [])); }
