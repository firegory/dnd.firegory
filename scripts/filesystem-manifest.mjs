...
async function walk(relative) {
  ...
  if (stat.isDirectory()) {
    if (relative) entries.push({ pathBase64, type: "directory" });
  ...
  if (stat.isFile()) {
    entries.push({ pathBase64, type: "file", size: stat.size });
    return;
  }
  if (stat.isSymbolicLink()) {
    entries.push({ pathBase64, type: "symlink", targetBase64: Buffer.from(await readlink(absolute)).toString("base64") });
    return;
  }
