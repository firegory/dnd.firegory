...
Canonical content publication is a separate worker-owned write path. Review projects the immutable extraction candidate and database-owned chunk evidence into canonical entry, complete text sections, typed fields, and source/file/chunk citations; unsupported evidence-preserving projections fail closed. A type-qualified `<entryType>-<candidateKey>` identity is shared by display, CAS lookup, publication, merge, and unpublication. The app persists the exact active revision token displayed to the reviewer (including explicit absence), reserves a monotonic generation, and fsyncs the immutable command and outbox state outside its read-only canonical mount. Under the canonical fence, the worker freshly resolves the target and rejects a stale expectation before mutation. Readers support immutable v1 replacement deltas and v2 replacement/deletion deltas; deletion emission requires a v2 bootstrap declaring both schemas. Valid per-target deltas fold by greatest generation, while revisions and deltas use fsynced temporary files and atomic same-filesystem renames. The worker records terminal review outcomes as `publication-worker`, retaining the initiating reviewer separately.
...
Compendium relations are source-derived and fail closed. A relation is readable only when `compendium_import_links` supplies at least one evidence version that survives the caller's source predicate and deleted/publication checks. A relation with no provenance is an unpublished candidate, not a public/manual relation. The application does not currently support globally visible manual relations; adding them requires an explicit trusted visibility classification rather than treating absent provenance as public.
