...
describe("getEmbeddingConfig", () => {
  it("should return default OpenAI config when no env vars set", () => {
    withEmbeddingEnv(() => {
  ...
  it("should use EMBEDDING_API_KEY from environment", () => {
    withEmbeddingEnv(() => {
      process.env.EMBEDDING_API_KEY = "test-key-123";
      const config = getEmbeddingConfig();
  ...
  it("should use custom base URL from environment", () => {
    withEmbeddingEnv(() => {
      process.env.EMBEDDING_BASE_URL = "https://custom.api.example.com/v1/";
      const config = getEmbeddingConfig();
  ...
  it("should use custom model from environment", () => {
    withEmbeddingEnv(() => {
      process.env.EMBEDDING_MODEL = "custom-embed-v2";
      const config = getEmbeddingConfig();
