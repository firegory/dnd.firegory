...
import { projectSnapshotSpellCandidate } from "../../src/server/compendium/candidate-publication.ts";
...
const source = {
  ...
  files: [{ fileId: fileUuid, path: `sources/next-dnd-snapshot/files/${fileUuid}.pdf`, mediaType: "application/pdf", contentHash: `sha256:${"a".repeat(64)}` }],
} as const;
...
test("collector spell becomes a cited canonical candidate and NFS database projection", () => {
  ...
  assert.equal(revision.citations.length, 1);
  assert.equal(revision.text.plain.slice(revision.citations[0].startOffset, revision.citations[0].endOffset), revision.citations[0].quote);
  ...
  const [projection] = projectCanonicalRevisions("fixture", [revision], [{
    ...
    mediaType: "application/pdf", contentHash: source.files[0].contentHash, byteSize: 512,
  }]);
...
test("NFS database rows map to API/UI contract with PDF-only citation previews", async () => {
  const row = spellRow();
  ...
  const snapshotRow = { ...row, mime_type: "text/html" };
...
test("spell pages retain RU/EN, mobile, and print contracts", async () => {
  const [i18n, css] = await Promise.all([
