...
test("flat list count, filters, relation targets, and cursor remain inside one RBAC boundary", async () => {
  ...
  assert.match(detailStatements[0], /compendium_import_links evidence JOIN accessible_relation_evidence authorized/); assert.match(detailStatements[0], /accessible_relation_versions AS MATERIALIZED/); assert.match(detailStatements[0], /JOIN nfs_index_managed_sources managed_source/); assert.match(detailStatements[0], /JOIN nfs_index_managed_files managed_file/); assert.match(detailStatements[0], /PARTITION BY n\.entry_id, n\.source_id, n\.file_id, n\.edition, n\.language/); assert.match(detailStatements[0], /accessible_relation_evidence AS MATERIALIZED/); assert.match(detailStatements[0], /accessible_relational_targets AS MATERIALIZED/); assert.match(detailStatements[0], /LEFT JOIN accessible_relation_versions nfs_target/); assert.match(detailStatements[0], /relational_target\.canonical_source_id = flat\.canonical_source_id AND relational_target\.file_id::text = flat\.canonical_file_id/); assert.match(detailStatements[0], /authorized\.canonical_source_id = flat\.canonical_source_id AND authorized\.file_id::text = flat\.canonical_file_id/); assert.doesNotMatch(detailStatements[0], /relational_target\.source_id = flat\.source_id|authorized\.source_id = flat\.source_id/); assert.match(detailStatements[0], /CASE WHEN relational_target\.entry_type::text IN .* THEN nfs_target\.entry_id ELSE relational_target\.entry_id::text END/); assert.match(detailStatements[0], /CASE WHEN flat\.entry_id .* THEN 0 ELSE 1 END AS identifier_rank/); assert.match(detailStatements[0], /count\(\*\) OVER \(\) AS match_count/); assert.match(detailStatements[0], /flat\.match_count = 1/); assert.match(detailStatements[0], /s\.owner_user_id/);
});
...
test("managed NFS provenance bridges canonical IDs to the exact relational target", async () => {
  const targetId = "44444444-4444-4444-8444-444444444444";
  const managedSourceId = "77777777-7777-4777-8777-777777777777", managedFileId = "88888888-8888-4888-8888-888888888888";
  const originalSourceId = sourceUuid, originalFileId = fileUuid;
  assert.notEqual(managedSourceId, originalSourceId); assert.notEqual(managedFileId, originalFileId);
  ...
  const relation = { type: "references", direction: "outgoing", entryId: targetId, entryType: "creature", title: "Goblin" };
  const flatRow = { ...synced, source_id: managedSourceId, file_id: managedFileId, canonical_source_id: source.sourceId, canonical_file_id: originalFileId, mime_type: source.files[0].mediaType, source_title: source.title, publication_code: source.publication.code, publication_revision: source.publication.revision, source_priority: 10, sort_title: "observant", source_versions: [], relations: [relation] };
  let flatSql = "";
  ...
  assert.match(flatSql, /managed_source\.canonical_source_id/); assert.match(flatSql, /managed_file\.canonical_file_id/); assert.match(flatSql, /relational_target\.canonical_source_id = flat\.canonical_source_id/); assert.match(flatSql, /authorized\.edition = flat\.edition AND authorized\.language = flat\.language/); assert.match(flatSql, /s\.access_tier = 'open'/);

  let genericValues: readonly unknown[] = [];
  const generic = new CompendiumReadService({ async query(_sql: string, values: readonly unknown[] = []) { genericValues = values; return { rows: [{ entry_id: targetId, canonical_key: "goblin", entry_type: "creature", edition: "5.5e", language: "ru", version_id: "55555555-5555-4555-8555-555555555555", revision_id: "66666666-6666-4666-8666-666666666666", slug: "goblin", aliases: [], title: "Goblin", summary: null, body: "A goblin.", extension_data: {}, projection: {}, relations: [], sources: [], citations: [], source_id: originalSourceId, source_title: "Open Rules", source_category: "core_rules", publication_code: "SRD", publication_title: "Open Rules", publisher: null, release_year: 2024, publication_revision: null, external_origin_url: null, attribution: null, license: null }] } as never; } });
  ...
  for (const mismatch of [{ canonical_source_id: "different-source", canonical_file_id: originalFileId }, { canonical_source_id: source.sourceId, canonical_file_id: "99999999-9999-4999-8999-999999999999" }]) {
    const visible = mismatch.canonical_source_id === source.sourceId && mismatch.canonical_file_id === originalFileId;
    const hidden = await new FlatReadService({ async query() { return { rows: [{ ...flatRow, ...mismatch, relations: visible ? [relation] : [] }] }; } }).get({ role: "user" }, "feat", synced.entry_id, { edition: "5.5e", language: "ru" });
