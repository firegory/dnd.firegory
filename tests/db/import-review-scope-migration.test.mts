...
test("review scope migration is additive, ordered, rerunnable, and nullable for existing runs", () => {
  assert.equal(MIGRATION_FILENAMES[MIGRATION_FILENAMES.indexOf("0022_import_review_canonical_revision.sql") + 1], "0023_import_run_review_scope.sql");
  assert.match(sql, /ADD COLUMN IF NOT EXISTS allowed_review_entry_types text\[\]/);
  assert.match(sql, /allowed_review_entry_types IS NULL/);
  assert.match(sql, /cardinality\(allowed_review_entry_types\) > 0/);
  assert.match(sql, /allowed_review_entry_types <@ ARRAY/);
