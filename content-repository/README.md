...
Each `manifests/activations/<generation>.json` replaces exactly one target entry. Readers validate the declared activation-delta JSON Schema, fold valid deltas over bootstrap `repository.json`, and keep the greatest generation independently for each target. Therefore a late older command cannot regress the same target, while commands for different targets compose. Corrupt deltas, mismatched filename generations, and malformed filenames are inert.

Allocation scans durable reservation filenames, commands, and activation filenames and retries exclusive creation after collisions. Reservation filenames remain authoritative after a partial write, so spool or database process rebuilds cannot reuse a generation. Only exact 32-digit filenames participate; malformed high filenames cannot exhaust allocation. `manifests/repository.json` remains bootstrap state and is never replaced by publication.
