...
Allocation scans self-validating checksummed reservation records, valid commands, and valid activation deltas, then retries exclusive installation after collisions. A reservation is fsynced under a temporary name before an exclusive hard link makes the complete record visible. Complete reservations from the earlier unchecksummed format are atomically upgraded with a checksum during scanning; partial or otherwise invalid records are quarantined and ignored, including a corrupt all-nines filename, so they cannot exhaust generation allocation. Valid pending reservations remain authoritative across process or database rebuilds. Only exact 32-digit generations participate. `manifests/repository.json` remains bootstrap state and is never replaced by publication.
