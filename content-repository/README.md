...
Activation contracts are immutable and versioned. Version 1 deltas replace exactly one target and never accept `entry: null`; version 2 adds target deletion through an explicit null entry. A version 2 bootstrap declares both schemas, and the current resolver folds both versions by per-target generation. Readers first validate the bootstrap and declarations, then validate deltas independently and finally validate only composed entries and revisions. Corrupt or unsupported deltas are inert.
...
Supported consumers must use `loadResolvedRepositoryManifest` rather than treating `manifests/repository.json` as active state. Roll out deletion support in two phases: first deploy readers that accept v1 and v2 while the repository remains v1, then upgrade the bootstrap and declarations to reader contract v2. Unpublication submission fails closed until that upgrade is visible, so no deletion delta can be emitted while old-reader compatibility is still required. Repository tooling may read the bootstrap directly only for descriptor metadata.

Queue deliveries have unique IDs and visibility deadlines. The review response returns each target's active revision token, including explicit absence; the client sends that displayed token unchanged into durable review state and the immutable command. Mutation handling never recaptures it. Under the canonical fence, the worker compares the token with freshly resolved active state before mutation, so stale cross-run approvals and removals fail permanently. Workers renew visibility with an ownership-checked Redis operation throughout publication. Acknowledgement, retry, renewal, and dead-letter transitions compare the current reservation ID; periodic recovery requeues only expired reservations and never drains another live worker's processing set. Lock contention requeues without consuming the bounded failure budget. Other transient failures use bounded exponential retry. Malformed deliveries, missing spool commands, schema/integrity failures, and exhausted retries are removed from active processing and recorded in both the Redis dead-letter list and the durable spool quarantine.
