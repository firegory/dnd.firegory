...
Redis leases coordinate delivery attempts, but they are not the canonical write fence: expiration alone cannot stop a paused writer. Before any canonical mutation, a worker acquires a PostgreSQL session advisory lock scoped to the repository ID and holds that dedicated database connection through manifest activation. A paused process retains the lock, while process or connection death releases it. Therefore a worker with a newer Redis lease cannot overtake a stale writer. PostgreSQL availability is required for publication even though canonical bytes remain filesystem-owned.

Queue deliveries have unique IDs and visibility deadlines. Acknowledgement, retry, and dead-letter transitions compare the current reservation ID; periodic recovery requeues only expired reservations and never drains another live worker's processing set. Transient failures use bounded exponential retry. Malformed deliveries, missing spool commands, schema/integrity failures, and exhausted retries are removed from active processing and recorded in both the Redis dead-letter list and the durable spool quarantine.

The spool is a durable filesystem outbox. Command and state files are fsynced, followed by their containing directory, before Redis enqueue. Workers periodically reconcile missing, pending, or stale queued state and redeliver with a new delivery ID. Completion is durably recorded before acknowledgement. Duplicate deliveries are harmless because the stable command key and deterministic revision identity are checked again by the worker.

A revision is written and fsynced to a unique temporary file under `.publication-staging/<entryId>/`, atomically renamed to deterministic staging, and then renamed to its deterministic immutable revision path. Each rename is followed by directory fsync. While fenced, retries remove abandoned temporary files and reuse byte-identical staged or final revisions. Readers discover revisions only through `manifests/repository.json`. Activation writes and fsyncs a unique complete temporary manifest in `manifests/`, then atomically renames it over `repository.json`; a crash before that rename leaves the previous manifest active.
