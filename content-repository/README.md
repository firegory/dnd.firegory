...
Allocation computes its floor from semantically valid commands and activation deltas using the same revision/source validation as the reader. Starting above that floor, it chooses the first generation whose reservation filename is absent. Every exact 32-digit reservation filename is a permanent consumed tombstone regardless of its bytes, so a live partial writer or corrupt record can never permit reuse; far corrupt names, including all-nines, are skipped only if reached and cannot force exhaustion. A new reservation is fsynced under a temporary name before an exclusive hard link makes the complete record visible, and collisions trigger a rescan. Complete reservations from the earlier unchecksummed format are atomically upgraded in place. `manifests/repository.json` remains bootstrap state and is never replaced by publication.
