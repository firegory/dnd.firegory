...
Migration `0016_compendium_guide_candidate_type.sql` adds only the `guide` enum discriminator required by durable import candidates. It is ordered after #79's `0014_compendium_entry_editor.sql` and #86's `0015_spells_vertical_slice.sql`. Guides remain version-controlled application content, category visibility uses the existing compendium index and centralized source authorization, and embeddings are not read or changed by these routes.
...
Compendium entry citations show quote, source, page, and section. Preview links prefer the cropped `chunkId` path and include page-level fallback coordinates; both paths use the authenticated `/api/citations/preview` endpoint and reapply centralized source authorization. External source links are emitted only for credential-free HTTP(S) publication origins.
