...
Entries are projected deterministically in entry-ID order by an explicitly versioned projector. UUIDs for newly managed sources, files, ingestion generations, documents, pages, chunks, and structured index entries are content-derived. When the canonical source or file already exists, sync locks and validates its corpus, access, publication, and content metadata, reuses its original UUID, and records `owns_source=false` or `owns_file=false`. Conflicting canonical identities fail the transaction instead of merging access tiers or publication metadata. The projector version is part of generation identity, so a projector change creates a fresh projection generation. Clean and changed-file activation reconciles that generation by deleting surplus rows marked `managedBy: nfs-content-index`; unmanaged rows are never selected. Embeddings are deliberately absent from this projection. An unchanged incremental run performs no database writes and preserves existing embeddings.
...
The mapping tables distinguish rows created by sync from reused relational rows. Before first activation on a reused file, sync retains its prior active generation and preserves that snapshot across incremental NFS generations. It also tracks the last NFS generation: first activation requires the file pointer to equal the retained predecessor, and incremental activation requires it to equal that last NFS generation. Both activation and removal lock the file/mapping and use compare-and-swap updates, so an externally changed pointer or concurrent claimant fails the transaction before existing content is archived. Removal restores the retained valid generation (or `NULL`), then removes all NFS-derived entries and their generation cascade. Owned files and their derived generations are deleted by cascade, while reused source/file rows remain. Canonical source/file IDs in these mappings bridge NFS entries back to relational compendium versions and import-link provenance.
