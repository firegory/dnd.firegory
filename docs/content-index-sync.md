...
A partial unique index permits only one staging/applying run per repository. Claim creation is one conflict-safe statement, matching concurrent callers share the same run, and an advisory lock serializes activation. Run status moves monotonically from `staging` to `applying` and then `succeeded` or `failed`; terminal runs cannot be reopened. Staging is not visible to retrieval. Final source/file/generation/document/page/chunk activation, exact managed-row reconciliation, and entry retirement occur in one PostgreSQL transaction. A failure rolls back the entire active-index change. An interruption during staging leaves the nonterminal checkpoints resumable; an activation failure is terminal and a later invocation claims a new run.
...
Projector version 2 requires every revision's citations to refer to one source file. It preserves every citation's page, section, quote, and offsets in page/chunk metadata. A canonical revision citing multiple files is rejected before database access rather than silently dropping provenance.
