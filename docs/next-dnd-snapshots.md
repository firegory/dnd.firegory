...
Snapshot candidates remain outside publication until the type-specific adapter produces a complete typed projection with immutable detail/index fingerprints and field citations. Current spell, creature, class, species, background, feat, glossary, item, and equipment projections are revalidated across the review boundary and can be `publishable`; incomplete or unsupported collector payloads remain `requires_extraction`. Approval is always an explicit administrator action and collection never publishes.
